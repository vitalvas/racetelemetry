...
func TestConvertTelemetry(t *testing.T) {
	...
	t.Run("throttle brake encoding", func(t *testing.T) {
		...
		// Unfiltered throttle at offset 13
		assert.Equal(t, uint8(255), data[13])
		// Unfiltered brake at offset 14
...
func TestConvertTimings(t *testing.T) {
	t.Run("timing data", func(t *testing.T) {
		...
		// Position X at offset header+1+4+4+4+4 = header+17
...
func BenchmarkConvertGameState(b *testing.B) {
	frame := &model.TelemetryFrame{IsRaceOn: true}

	b.ResetTimer()

	for b.Loop() {
		convertGameState(1, frame)
	}
}

func BenchmarkConvertTimings(b *testing.B) {
	frame := &model.TelemetryFrame{
		IsRaceOn:       true,
		PositionX:      100.0,
		LapDistance:    1500.0,
