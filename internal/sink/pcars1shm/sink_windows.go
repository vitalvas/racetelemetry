...
func New() (*Sink, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pcars1shm: utf16 name: %w", err)
	}
	...
	if handle == 0 {
		return nil, fmt.Errorf("pcars1shm: CreateFileMapping: %w", err)
	}
	...
	if addr == 0 {
		...
		return nil, fmt.Errorf("pcars1shm: MapViewOfFile: %w", err)
	}
